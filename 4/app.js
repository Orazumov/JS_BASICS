...
buttons.forEach(button => {button.addEventListener('click', function(event) {handleClick(event)})} )
...
function handleClick(clickedButtonEvent) {
    ...
    const button_text = card.button.innerHTML;
    
    if (button_text == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"}
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card)}
    else if (button_text == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    const random_text = 'This is text, item description. Best item on the world!';
